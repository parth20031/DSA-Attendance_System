...
def interface():
    ...
    while True:
        ...
        for i in range(3):
                ...
                he,we,_=part.shape
                part=cv2.resize(part,(3*we,3*he))

                # part=cv2.resize(part,(640,486))
                part=cv2.cvtColor(part,cv2.COLOR_BGR2RGB)
                temp=face_recognition.face_locations(part)
                allFacesInParts.append(temp)
                # for faceloc in temp:
                #     face=part[faceloc[0]:faceloc[2],faceloc[3]:faceloc[1]]
                #     face_deblurred = cv2.GaussianBlur(face, (0, 0), 3) 
                #     part[faceloc[0]:faceloc[2],faceloc[3]:faceloc[1]]=face_deblurred
